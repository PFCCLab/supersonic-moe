...
def default_config(device, num_experts: int = 1) -> GemmConfig:
    ...
    # SM103 2-CTA cluster tiling for the backward dgated GEMM, gated by
    # SONIC_MOE_DGATED_2CTA. DEFAULT OFF ("0"): the conservative 1-CTA tile
    # (cluster_m=1, tile_m=128) is used unless the env var is explicitly set to
    # an on-value. The 2CTA path (cluster_m=2, tile_m=128) is still reachable
    # via SONIC_MOE_DGATED_2CTA=1 for opt-in benchmarking, but it has not been
    # qualified end-to-end on SM103 (an illegal-memory-access fault surfaced in
    # the DeepEP dispatch backward under a full training step), so it stays off
    # by default until that is root-caused. No effect off SM103.
    import os as _os
    if _os.getenv("SONIC_MOE_DGATED_2CTA", "0").lower() not in {"1", "true", "yes", "on"}:
        return GemmConfig(
            tile_m=128,
            tile_n=128,
            cluster_m=1,
    ...
    if cap[0] == 10 and cap[1] >= 3:
        ...
        # 2-CTA cooperative cluster tiling which the prior cluster_m=1 override
        # disabled.  Empirically (nsys GPU-projection, fresh proc/cfg):
        # cluster_m=2 is ~6-14% faster on the dgated kernel at production shapes
        # (E<=64) and bit-identical (rrmse=0; tile/cluster partition does not
        # change the K-reduction order) with zero extra memory.
        ...
        # tile_m is ALWAYS 128 here. Two reasons, both hard constraints:
        #   1. Route-level padding aligns every expert's M to FP8_ALIGN=128
        #      (fusion_layer_utils.padding_token_per_experts). We can only
        #      guarantee a clean tiling at tile_m=128; tile_m=256 would need
        #      256-aligned padding which the fleet does not provide.
        #   2. tile_m=256 + cluster_m=2 is the ONLY config that sets
        #      use_2cta_instrs=True (quack gates it on mma_tiler_m==256), which
        #      in turn produces the (2,2) tmem epilogue warp shape and the
        #      silent gated-postact corruption of Dao-AILab issue #63 / quack
        #      PR #133. Pinning tile_m=128 keeps cta_tile_m=128 and the (4,1)
        #      warp shape, so that corruption path is unreachable by construction
        #      — no epilogue-copy override is needed.
        # cluster_m=2 with tile_m=128 gives a 2-CTA *cluster* (cooperative
        # load/tiling) WITHOUT the 2-CTA tcgen05 MMA, i.e. the safe speedup.
        tile_m = 128
        return GemmConfig(
            tile_m=tile_m,
            tile_n=128,
