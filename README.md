...
- **quack import path**: set `SONIC_MOE_QUACK_PATH` when the active `python` lacks the `quack` site-package. `tests/conftest.py` injects this into `sys.path` automatically; `tools/ci/jit_bench.py::_run_subprocess` and `tools/ci/multicard_smoke.WORKER_BODY` do the same for spawned workers.
