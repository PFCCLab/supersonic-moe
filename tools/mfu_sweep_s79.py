...
Hardware: NVIDIA Target GPU (SM100-class, sm_103). FP8 dense tensor-core peak
...
PEAK_TFLOPS_FP8 = 4500.0          # Target GPU dense tensor-core FP8 peak (TFLOP/s)
...
def render_plots(rows: list[dict], outdir: Path) -> None:
    ...
    if not big.empty:
        ...
        ax.set_ylabel("MFU (%)")
        ax.set_title(
            "FP8 Frontier MFU vs Model Width @ T=8192 — single Target GPU, FP8 peak 4500 TFLOPS"
        )
        for p in ax.patches:
            v = p.get_height()
            if v > 0:
...
def write_readme(rows: list[dict], outdir: Path) -> None:
    ...
    md.append("> Generated by `tools/mfu_sweep_s79.py`. Hardware: NVIDIA Target GPU "
              "(SM100-class, sm_103). FP8 dense tensor-core peak = "
